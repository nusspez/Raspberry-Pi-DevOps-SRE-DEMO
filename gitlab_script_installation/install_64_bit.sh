...
GITLAB_URL="https://192.168.100.16"
# Token de registro para runners (lo obtienes en la sección de runners de tu proyecto o instancia)
REGISTRATION_TOKEN="YourRunnerRegistrationToken"
...
# Crear un token de API para root (esto es solo un ejemplo; en la práctica, podrías usar el token existente o generar uno desde la interfaz)
...
PROJECT_INFO=$(curl --silent --header "PRIVATE-TOKEN: $PRIVATE_TOKEN" "$GITLAB_URL/api/v4/projects/$PROJECT_ID")
